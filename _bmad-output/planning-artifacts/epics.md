...
- shadcn/ui CLI 설치 및 컴포넌트 선택적 추가 (Card, Table, Badge, Popover, Breadcrumb, Skeleton, Tabs, Button)
...
UX-DR5: ChampionTierTable 커스텀 컴포넌트 구현 — 티어 섹션 접기/펼치기, 컬럼 정렬, shadcn Table 기반
UX-DR6: EloLeaderboard 커스텀 컴포넌트 — 데스크톱(테이블)/모바일(카드 리스트) 변형, 자신의 행 bg-teal-900/20 하이라이트
UX-DR7: StatHoverWidget(PlayerLink/ChampionLink) 스타일 업데이트 — 기존 인터페이스(riotId, children, className, mode) 유지, Popover 기반으로 스타일 개선
UX-DR8: BanRecommendBadge 커스텀 컴포넌트 — 높은 위협도(#EF4444)/일반(#00B4D8), shadcn Badge 기반
...
UX-DR14: Breadcrumb 컴포넌트 상단 고정 — "전체 > 챔피언 > 야스오" 형태, 각 항목 클릭 가능, shadcn Breadcrumb 기반
...
**Architecture requirements:** shadcn/ui 설치, React Query QueryClient 설정, 신규 폴더 구조, App.tsx 라우팅 재구성
...
So that Card, Table, Badge, Popover, Breadcrumb, Skeleton, Tabs, Button 등 shadcn 컴포넌트를 즉시 사용할 수 있다.

**Acceptance Criteria:**

**Given** `frontend/` 디렉토리에서 shadcn/ui CLI를 실행했을 때
**When** 필요한 컴포넌트들이 추가되면
**Then** `components/ui/` 폴더에 card.tsx, table.tsx, badge.tsx, popover.tsx, breadcrumb.tsx, skeleton.tsx, tabs.tsx, button.tsx가 존재한다

**Given** 기존 Tailwind CSS 설정이 있을 때
**When** shadcn/ui 컴포넌트를 import하면
**Then** 기존 스타일과 충돌 없이 렌더링된다

**Given** Lucide React가 shadcn/ui 기본 아이콘으로 설치되었을 때
...
**Given** shadcn Badge 컴포넌트 기반으로 구현되었을 때
**When** 컴포넌트 구조를 확인하면
**Then** shadcn Badge를 커스텀한 형태다
