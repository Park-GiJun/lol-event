    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 'var(--spacing-sm)' }}>
          ...
          return (
            <div key={`${cert.riotId}-${cert.champion}`} className="card"
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)', marginBottom: 'var(--spacing-sm)' }}>
                <ChampImg championId={cert.championId} champion={cert.champion} size={40} style={{ borderRadius: 'var(--radius-md)', boxShadow: '0 0 10px rgba(var(--color-primary-rgb), 0.15)' }} />
                <div>
        ...
        {!data.certifiedMasters.length && (
          <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)', gridColumn: '1/-1' }}>
            인증된 장인이 없습니다
          </p>
        )}
      </div>
