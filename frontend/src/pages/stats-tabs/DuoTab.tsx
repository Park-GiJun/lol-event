              ...
              <tr key={`${d.player1}-${d.player2}`} className="member-stats-row">
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-sm)' }}>
                    ...
                    <span style={{ color: 'var(--color-primary)', fontSize: 'var(--font-size-xs)', fontWeight: 700 }}>+</span>
                ...
                <td className="table-number" style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-secondary)' }}>
                  {d.avgKills.toFixed(1)} / <span style={{ color: 'var(--color-loss)' }}>{d.avgDeaths.toFixed(1)}</span> / {d.avgAssists.toFixed(1)}
                </td>
              </tr>
            ))}
            {!data.duos.length && (
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: 'var(--spacing-2xl) 0', color: 'var(--color-text-secondary)' }}>
