...
object RankingScore {
    ...
    /**
     * 목록에 올리기 위한 최소 표본. 조합 지표든 개인 랭킹이든 3경기로 통일한다.
     *
     * 컷을 낮게 잡아도 되는 이유는 **정렬을 [shrunkWinRate] 가 맡기 때문**이다.
     * 컷은 "무엇을 보여줄지", 축소 보정은 "누가 위인지"를 각각 담당한다.
     * 예전에는 정렬까지 관측 승률로 해서 1경기 1승이 33경기 20승을 이겼다. 그게 진짜 문제였고,
     * 컷을 올리는 건 그 증상을 가리는 방식이었을 뿐이다.
     *
     * 154경기 실측 표본 분포는 이렇다. 3경기로 자르면 목록이 비지 않으면서도
     * 한 판짜리 100% 승률은 정렬에서 자연히 아래로 내려간다.
     *   같은 팀 듀오      735조합, 중앙 2경기
     *   상대 라인 대결  1,045조합, 중앙 4경기
     *   플레이어x챔피언  681조합, 중앙 1경기
     */
